...
bias_gyro_x = 0.09 # [rad/measurement]
...
for line in f:
	...
	predAngle = estAngle + gyro_x
	gyroVarAcc = gyroVarAcc + acc_x
	predVar = estVar + gyroVarAcc
	estAngle = predAngle
	estVar = predVar

	# Kalman correction step (we have new data in each iteration)
	K = predVar / (predVar + acc_x)
	corrAngle = predAngle + K * (gyro_x_rel - predAngle)
	corrVar = predVar * (1 - K)
	estAngle = corrAngle
	estVar = corrVar
	gyroVarAcc = 0.0

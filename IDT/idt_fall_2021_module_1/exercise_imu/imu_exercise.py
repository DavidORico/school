...
for line in f:
	...
	#3.1
	#myValue = atan2(acc_y, sqrt(acc_x*acc_x + acc_z*acc_z))
	#3.2
	#myValue = atan2(-acc_x, acc_z)
	#3.3
	#myValue = [atan2(acc_y, sqrt(acc_x * acc_x + acc_z * acc_z)) *180.0/pi, atan2(-acc_x, acc_z) *180.0/pi]
	#3.4
	alpha = 0.05
	x = atan2(acc_y, sqrt(acc_x * acc_x + acc_z * acc_z))
	lpf += alpha * (x - lpf)
	myValue = [x*180.0/pi, lpf*180.0/pi]
	#3.3.1, 3.3.2
	#myValue += gyro_z * (ts_now - ts_prev)
	#3.3.3
	#bias_z = 0.00000777
	#myValue += gyro_z * (ts_now - ts_prev) - bias_z
...
if showPlot == True:
	...
	plt.savefig('imu_exercise_plot_num_int_static_with bias.png')
	plt.show()
